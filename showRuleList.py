...
if __name__ == '__main__':
    ...
    file_in = "NumericalData/skin/Saved_Trees/Pruned/PruneTree0_trial0_5-5_20190907-1246.pkl" # ECDT tree file
    with open(file_in, 'rb') as f:
        tree = pickle.load(f)
    weight, bias = load_weight_bias_path('NumericalData/skin/Model/DNNmodel0_trial0_5-5_20190907-1246.h5') # DNN file
    rule_list_txt, rule_list = extractRules(tree,hidden_nodes,num_input,num_output,weight,bias)

    total_constraints = 0
    for i in range(0,len(rule_list)):
        total_constraints = total_constraints + len(rule_list[i])
        
    print("Number of rules = " + str(len(rule_list)))
    print("Total number of constraints = " + str(total_constraints))
